...
const ProfileSection: React.FC = () => {
  ...
  const { data: profileResponse, isLoading } = useQuery({
    ...
    enabled: true, // Always enabled, API uses SecurityContext to get current user
  });
  ...
  useEffect(() => {
    if (profileResponse) {
      setFormData({
        ...
        dob: profileResponse.dob ? profileResponse.dob.split('T')[0] : '', // Convert date to YYYY-MM-DD format
      });
    }
  }, [profileResponse]);
  ...
  const updateProfileMutation = useMutation({
    ...
    onSuccess: (result) => {
      if (result) {
        ...
        // Email is not updated (read-only field)
